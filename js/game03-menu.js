(() => {
  ...
  function ensureUi(){
    ...
    if(fieldWindow&&!$("#field-main-menu")){
      ...
      menu.setAttribute("aria-label","フィールドメニュー");
      menu.innerHTML=`
        <div class="field-main-menu-title">MENU</div>
        <div id="field-main-menu-items" class="field-main-menu-items"></div>
        <div class="field-main-menu-help">↑↓ 選択　Enter 決定　Z 閉じる</div>`;
  ...
  function openFieldMenu(){
    if(!fieldScreenActive()||dialogIsOpen())return;
    fieldMenuOpen=true;
    fieldMenuIndex=0;
    renderFieldMenu();
  ...
  function activateFieldMenu(index=fieldMenuIndex){
    const item=fieldMenuItems[index];
    if(item)item.action();
  }
  ...
  function onFieldMenuKeydown(event){
    if(fieldMenuOpen){
      if(isZKey(event)||event.key==="Escape"){
        event.preventDefault();event.stopImmediatePropagation();closeFieldMenu();return;
      }
    ...
    if(fieldScreenActive()&&isZKey(event)&&!dialogIsOpen()){
      event.preventDefault();event.stopImmediatePropagation();openFieldMenu();
    }
  }
  ...
  window.SpellMenu={renderStatus,renderLoadout,openStatus,openFieldMenu,closeFieldMenu,toggleFieldMenu,isOpen};
  renderLoadout();
})();
