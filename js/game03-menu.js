(() => {
  ...
  let travelMenuOpen=false,travelMenuIndex=0;
  ...
  const fallbackDestinations=[{id:"la_mer_city",name:"ラメールシティ"}];

  function travelDestinations(){
    const items=window.SpellTravel?.destinations?.();
    return Array.isArray(items)&&items.length?items:fallbackDestinations;
  }
  ...
  function ensureUi(){
    ...
    if(fieldWindow&&!$("#field-main-menu")){
      ...
      menu.innerHTML=`<div class="field-main-menu-title">MENU</div><div id="field-menu-money" class="field-main-menu-money">0 G</div><div id="field-main-menu-items" class="field-main-menu-items"></div><div class="field-main-menu-help">↑↓ 選択　Z 決定　Enter 閉じる</div>`;
      fieldWindow.appendChild(menu);
    }
    if(fieldWindow&&!$("#field-travel-menu")){
      const menu=document.createElement("div");
      menu.id="field-travel-menu";
      menu.className="field-main-menu hidden";
      menu.setAttribute("role","menu");
      menu.innerHTML=`<div class="field-main-menu-title">イードウ</div><div id="field-travel-menu-items" class="field-main-menu-items"></div><div class="field-main-menu-help">↑↓ 選択　Z イードウ　Enter 戻る</div>`;
      fieldWindow.appendChild(menu);
    }
  ...
  function renderFieldMenu(){
    ...
    box.innerHTML=fieldMenuItems.map((item,index)=>`<button type="button" class="field-main-menu-item${index===fieldMenuIndex?" selected":""}" data-field-menu-index="${index}"><span class="menu-cursor">▶</span><span>${item.label}</span></button>`).join("");
  }

  function renderTravelMenu(){
    const box=$("#field-travel-menu-items");
    if(!box)return;
    const destinations=travelDestinations();
    travelMenuIndex=Math.max(0,Math.min(travelMenuIndex,destinations.length-1));
    box.innerHTML=destinations.map((item,index)=>`<button type="button" class="field-main-menu-item${index===travelMenuIndex?" selected":""}" data-travel-menu-index="${index}"><span class="menu-cursor">▶</span><span>${item.name}</span></button>`).join("");
  }
  ...
  function isOpen(){return fieldMenuOpen||travelMenuOpen;}

  function openFieldMenu(){
    if(!fieldScreenActive()||dialogIsOpen()||storyOverlayIsOpen())return;
    travelMenuOpen=false;
    $("#field-travel-menu")?.classList.add("hidden");
    fieldMenuOpen=true;
    fieldMenuIndex=0;
    renderFieldMenu();
  ...
  function openTravelMenu(){
    ...
    travelMenuIndex=0;
    renderTravelMenu();
    $("#field-travel-menu")?.classList.remove("hidden");
  }

  function closeTravelMenu({returnToMain=false}={}){
    travelMenuOpen=false;
    $("#field-travel-menu")?.classList.add("hidden");
    if(returnToMain){
      fieldMenuOpen=true;
      fieldMenuIndex=Math.max(0,fieldMenuItems.findIndex(item=>item.label==="イードウ"));
      renderFieldMenu();
      $("#field-main-menu")?.classList.remove("hidden");
    }
  }
  ...
  function toggleFieldMenu(){
    if(travelMenuOpen){closeTravelMenu({returnToMain:true});return;}
    fieldMenuOpen?closeFieldMenu():openFieldMenu();
  }

  function moveFieldMenu(delta){
    fieldMenuIndex=(fieldMenuIndex+delta+fieldMenuItems.length)%fieldMenuItems.length;
    renderFieldMenu();
  }

  function moveTravelMenu(delta){
    const destinations=travelDestinations();
    if(!destinations.length)return;
    travelMenuIndex=(travelMenuIndex+delta+destinations.length)%destinations.length;
    renderTravelMenu();
  }

  function activateFieldMenu(index=fieldMenuIndex){fieldMenuItems[index]?.action?.();}

  function activateTravelMenu(index=travelMenuIndex){
    const destination=travelDestinations()[index];
    if(!destination)return;
    closeAllMenus();
    if(window.SpellTravel?.warpTo?.(destination.id))return;
    window.SpellField?.activateMap?.(destination.id,{from:"fast-travel"});
    window.SpellPlaces?.refresh?.();
  }

  function statRows(stats,key){
    const p=state.party[key],rows=[["HP",`${p.hp} / ${stats.hp}`],["MP",`${p.mp} / ${G.maxMpFor(stats,p.level)}`],["攻撃",stats.attack],["防御",stats.defense],["特攻",stats.spAttack],["特防",stats.spDefense],["素早さ",stats.speed]];
    return rows.map(([name,value])=>`<div><dt>${name}</dt><dd>${value}</dd></div>`).join("");
  }

  function memberCard(key){
    const progress=state.party[key],species=G.partySpecies[key],stats=G.getMemberStats(key),next=G.expToNext(progress.level),pct=Math.max(0,Math.min(100,(progress.exp/next)*100)),equipKey=state.equipment[key],equip=G.itemDefinitions[equipKey],equipText=equip?`${equip.name}（${equip.description}）`:"装備なし";
  ...
  function openPluginPrompt(){
    if(!fieldScreenActive()||dialogIsOpen()||storyOverlayIsOpen()||isOpen())return false;
  ...
  function onFieldMenuKeydown(event){
    ...
    if(travelMenuOpen){
      if(isEnterKey(event)||event.key==="Escape"){
        ...
        closeTravelMenu({returnToMain:true});
        return;
      }
    ...
    if(isZKey(event)&&window.SpellDialogTyping?.handleAdvance?.()){
      event.preventDefault();
      event.stopImmediatePropagation();
      return;
    }
    if(isZKey(event)&&continuePlugin()){
      event.preventDefault();
      event.stopImmediatePropagation();
      return;
    }
  ...
  $("#field-main-menu")?.addEventListener("click",event=>{
    const b=event.target.closest("[data-field-menu-index]");
    if(!b)return;
    fieldMenuIndex=Number(b.dataset.fieldMenuIndex)||0;
    activateFieldMenu(fieldMenuIndex);
  });
  $("#field-travel-menu")?.addEventListener("click",event=>{
    const b=event.target.closest("[data-travel-menu-index]");
    if(!b)return;
    travelMenuIndex=Number(b.dataset.travelMenuIndex)||0;
    activateTravelMenu(travelMenuIndex);
  });
