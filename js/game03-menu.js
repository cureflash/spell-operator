(() => {
  ...
  function activeFieldMenuItems() {
    if (fieldMenuMode === "travel") return travelMenuItems.map(item => ({
      label: item.label,
      action: () => requestTravel(item.mapId, item.label)
    }));
  ...
  function closeAuxScreenWithZ(event) {
    if (!isZ(event) || editable(event.target)) return false;
  ...
  function onKeydown(event) {
    if (closeAuxScreenWithZ(event)) return;
    ...
    if (fieldMenuOpen) {
      if (isEnter(event) || event.key === "Escape") {
        event.preventDefault();
        event.stopImmediatePropagation();
