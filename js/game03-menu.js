(() => {
  ...
  function activeFieldMenuItems(){if(fieldMenuMode==="travel")return travelMenuItems;if(fieldMenuMode==="settings")return settingsMenuItems();return fieldMenuItems}
  function ensureUi(){
  ...
  function returnFromSubMenu(){const label=fieldMenuMode==="travel"?"イードウ":"設定";fieldMenuMode="main";fieldMenuIndex=Math.max(0,fieldMenuItems.findIndex(item=>item.label===label));renderFieldMenu()}
  ...
  function activateFieldMenu(index=fieldMenuIndex){const item=activeFieldMenuItems()[index];if(item?.setting){adjustAudio(.1);return}item?.action?.()}
