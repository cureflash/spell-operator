(() => {
  ...
  function ensureUi(){
    ...
    const spellGrid=document.querySelector("#screen-hub .spell-grid");if(spellGrid&&!$("#spell-loadout")){const section=document.createElement("section");section.id="spell-loadout";section.className="panel spell-loadout";section.innerHTML=`<div class="spell-loadout-head"><div><p class="eyebrow">BATTLE LOADOUT</p><h3>実戦登録枠</h3></div><span id="loadout-count" class="badge muted">0 / 4</span></div><div id="spell-loadout-slots" class="loadout-slots"></div><div id="known-spell-list" class="known-spells"></div><p id="loadout-message" class="loadout-warning">解析済みの魔法を登録枠へセットしてください。</p>`;spellGrid.insertAdjacentElement("afterend",section)}
  }
  ...
  function renderLoadout(){const slots=$("#spell-loadout-slots"),known=$("#known-spell-list");if(!slots||!known)return;slots.innerHTML=state.spellSlots.map((key,index)=>{if(!key)return `<div class="loadout-slot empty"><span class="loadout-slot-number">SLOT ${index+1}</span><span>EMPTY</span></div>`;const spell=state.registeredSpells[key];return `<div class="loadout-slot"><span class="loadout-slot-number">SLOT ${index+1}</span><div><div class="loadout-spell-name">${spell?.name||key}</div><div class="loadout-spell-meta">MP ${spell?.mpCost??"—"}</div></div><button class="slot-remove" data-remove-slot="${index}">外す</button></div>`}).join("");const keys=Object.keys(state.registeredSpells);known.innerHTML=keys.length?keys.map(key=>{const spell=state.registeredSpells[key],equipped=G.isSpellEquipped(key);return `<div class="known-spell"><strong>${spell.name}</strong><span>MP ${spell.mpCost}</span><button class="secondary compact" data-equip-spell="${key}" ${equipped?"disabled":""}>${equipped?"登録中":"セット"}</button></div>`}).join(""):`<span class="muted">解析済みの魔法はまだありません。</span>`;const count=state.spellSlots.filter(Boolean).length,badge=$("#loadout-count");if(badge){badge.textContent=`${count} / 4`;badge.className=`badge ${count?"success":"muted"}`}const msg=$("#loadout-message");if(msg){const ready=G.magicReady();msg.textContent=ready?"Fire と Repair が実戦登録されています。":"この試作戦闘では Fire と Repair の両方を登録枠へセットしてください。";msg.className=ready?"loadout-warning loadout-ready":"loadout-warning"}}
  ...
  ensureUi();
  $("#field-menu")?.addEventListener("click",toggleFieldMenu);$("#field-main-menu")?.addEventListener("click",event=>{const b=event.target.closest("[data-field-menu-index]");if(!b)return;fieldMenuIndex=Number(b.dataset.fieldMenuIndex)||0;activateFieldMenu(fieldMenuIndex)});$("#status-back")?.addEventListener("click",closeStatus);$("#spell-loadout")?.addEventListener("click",event=>{const remove=event.target.closest("[data-remove-slot]");if(remove){G.unequipSlot(Number(remove.dataset.removeSlot));renderLoadout();return}const equip=event.target.closest("[data-equip-spell]");if(equip){G.equipSpell(equip.dataset.equipSpell);renderLoadout()}});document.addEventListener("keydown",onFieldMenuKeydown,true);
