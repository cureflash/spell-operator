(() => {
  ...
  const FRIEND_CIPHER="DBU",FRIEND_ANSWER="CAT",ENCRYPTED="080B07FE080907FA0802080B07F0",PASSWORD="REPAIR7";
  ...
  function shiftFriend(amount){const current=$("#friend-shift-text");if(!current||!Number.isFinite(amount)||amount===0)return;current.textContent=shiftUppercase(current.textContent,amount)}
  function openFriendPuzzle(){openOverlay(`<p class="eyebrow">CLASSMATE QUEST</p><h3>シーザー暗号クイズ</h3><p>アルファベットを全部同じ数だけずらす暗号を「シーザー暗号」という。矢印で暗号文そのものを動かして、規則を確かめてみよう。</p><div class="cipher-card friend-shift-card"><span>暗号文</span><div class="friend-shift-row"><code id="friend-shift-text">${FRIEND_CIPHER}</code><div class="friend-shift-buttons" aria-label="暗号文をずらす"><button type="button" class="secondary compact" data-friend-shift="1" aria-label="1文字先へずらす">↑</button><button type="button" class="secondary compact" data-friend-shift="-1" aria-label="1文字前へずらす">↓</button></div></div><small>↑で1文字先、↓で1文字前。Aの前はZ、Zの次はAになる。</small></div><label class="story-input-label">復号した単語<input id="friend-answer" autocomplete="off" spellcheck="false" placeholder="英大文字で入力"></label><p id="friend-result" class="story-result"></p><div class="story-actions"><button class="primary" id="friend-check">答え合わせ</button></div>`);setTimeout(()=>$("#friend-answer")?.focus(),0)}
  ...
  function checkFriend(){const answer=$("#friend-answer")?.value.trim().toUpperCase()||"";if(answer!==FRIEND_ANSWER){const r=$("#friend-result");if(r)r.textContent="まだ違う。↓を使ってDBUを1文字前へ戻してみよう。";return}story.friendSolved=true;story.stage=2;closeOverlay();refreshField();say({speaker:"classmate",text:`正解、${FRIEND_ANSWER}！　図書館のピジブルなら、文字コードを使う暗号も知ってるよ。町へ戻って北側の図書館に行ってみて。`})}
  ...
  function handleNpc(id){
    if(id==="classmate"){
      if(story.stage===0){story.stage=1;refreshField();say({speaker:"classmate",text:`ソフィー、シーザー暗号って知ってる？　今回は1文字だけ先にずらしたよ。A→B、B→C。じゃあ「${FRIEND_CIPHER}」を元に戻せる？`});return true}
