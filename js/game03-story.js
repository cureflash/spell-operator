(() => {
  ...
  function openFriendPuzzle(){openOverlay(`<p class="eyebrow">CLASSMATE QUEST</p><h3>シーザー暗号クイズ</h3><p>アルファベットを全部同じ数だけずらす暗号を「シーザー暗号」という。今回は1文字だけ先へずらしてある。</p><div class="cipher-card"><span>暗号文</span><code>${FRIEND_CIPHER}</code><small>A→B、B→C。1文字前へ戻せば元の単語になる。</small></div><label class="story-input-label">復号した単語<input id="friend-answer" autocomplete="off" spellcheck="false" placeholder="英大文字で入力"></label><p id="friend-result" class="story-result"></p><div class="story-actions"><button class="primary" id="friend-check">答え合わせ</button></div>`);setTimeout(()=>$("#friend-answer")?.focus(),0)}
  ...
  function checkFriend(){const answer=$("#friend-answer")?.value.trim().toUpperCase()||"";if(answer!==FRIEND_ANSWER){const r=$("#friend-result");if(r)r.textContent="まだ違う。DBUをそれぞれ1文字前へ戻してみよう。";return}story.friendSolved=true;story.stage=2;closeOverlay();refreshField();say({speaker:"classmate",text:`正解、${FRIEND_ANSWER}！　図書館のピジブルなら、文字コードを使う暗号も知ってるよ。町へ戻って北側の図書館に行ってみて。`})}
