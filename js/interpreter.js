  ...
  class SpellInterpreter {
    ...
    #normalize(source) {
      return source.replace(/\t/g, "    ").split(/\r?\n/).map((raw, index) => {
        ...
        return { lineNumber: index + 1, indent, text };
    ...
    #executeBlock(lines, startIndex, indent, state) {
      ...
      while (i < lines.length) {
        ...
        let match;
        if ((match = text.match(/^spell\s+([a-zA-Z_][\w]*)$/))) {
          if (state.spell !== null) throw new SpellError("spell 宣言は1回だけです。", line.lineNumber);
          state.spell = match[1].toLowerCase();
          i += 1;
          continue;
        }
        ...
        if ((match = text.match(/^for\s+([a-zA-Z_]\w*)\s+in\s+range\((.*)\):$/))) {
          const varName = match[1];
          const count = this.#evalExpression(match[2], state.variables, line.lineNumber);
          if (!Number.isInteger(count) || count < 0) throw new SpellError("range() は0以上の整数にしてください。", line.lineNumber);

          const bodyIndent = indent + 4;
          const bodyStart = i + 1;
          if (bodyStart >= lines.length || lines[bodyStart].indent !== bodyIndent) {
            throw new SpellError("for の中身を4スペース下げて書いてください。", line.lineNumber);
          }
          let bodyEnd = bodyStart;
          while (bodyEnd < lines.length && lines[bodyEnd].indent >= bodyIndent) bodyEnd += 1;
          const bodyLines = lines.slice(bodyStart, bodyEnd);
    ...
    #evalExpression(expression, variables, lineNumber) {
      ...
      for (const name of identifiers) {
        if (!(name in variables)) throw new SpellError(`変数 ${name} は定義されていません。`, lineNumber);
      }
      let jsExpr = expr;
      ...
      try {
        ...
        throw new SpellError("式を計算できません。", lineNumber);
      }
    }

    #step(state) {
      state.steps += 1;
      if (state.steps > this.stepLimit) throw new OverheatError(this.stepLimit);
    }
  }

  const api = { SpellInterpreter, SpellError, OverheatError };
  root.SpellRuntime = api;
