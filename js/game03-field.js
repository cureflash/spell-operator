(() => {
  ...
  function mapSpec(id){if(id==="school")return{...SCHOOL,blocked:schoolBlocked(),area:"学校"};if(id==="library")return{...LIBRARY,blocked:libraryBlocked(),area:"ピジブルの図書館"};return{...TOWN,blocked:townBlocked(),area:"はじまりの町"};}
  ...
  function clearWorld(world){[...world.querySelectorAll(".field-tile,.field-map-label,.school-marker,.story-npc,#field-shop,#field-unicode-item,.map-exit-marker,.library-marker")].forEach(el=>{if(el.id!=="field-npc")el.remove();});const classmate=$("#field-npc");if(classmate){classmate.classList.add("hidden");classmate.querySelector(".entity-tag")?.replaceChildren();}}
  ...
  function buildMap(){
    ...
    const frag=document.createDocumentFragment();for(let y=0;y<spec.height;y++)for(let x=0;x<spec.width;x++){const t=document.createElement("div");const type=fieldState.mapId==="school"?schoolTile(x,y):fieldState.mapId==="library"?libraryTile(x,y):townTile(x,y);t.className=`field-tile ${type}`;t.dataset.x=x;t.dataset.y=y;frag.appendChild(t);}world.insertBefore(frag,world.firstChild);
  ...
  function updateObjective(){const storyObjective=window.SpellStory?.objective?.();let text;if(storyObjective)text=storyObjective;else if(fieldState.enemyDefeated)text="実戦成功。町を探索しよう";else if(G.magicReady())text="東の草むらにいる魔物へ向かおう";else text="メニューのパソコンで Fire と Repair の魔導書に挑戦しよう";$("#field-objective").textContent=text;}
  ...
  function lumiereText(){if(!window.SpellStory?.isComplete?.())return"今の目的は画面上部に出てるよ。建物の入口に向かえば別マップへ入れる。";return fieldState.enemyDefeated?"今の戦い、悪くなかった。":G.magicReady()?"Fire と Repair は修得済み。東側で実戦テストしよう。":"パソコンで魔導書の問題を解こう。";}
  ...
  function activateMap(id,{snapshot=null,from=null,silent=false}={}){
    ...
    if(snapshot){model.restore(snapshot);}else if(id==="school"){model.restore({player:{x:7,y:10,facing:"up"},follower:{x:7,y:11,facing:"up"}});}else if(id==="library"){model.restore({player:{x:7,y:10,facing:"up"},follower:{x:7,y:11,facing:"up"}});}else if(from==="school"){model.restore({player:{x:3,y:4,facing:"down"},follower:{x:3,y:3,facing:"down"}});}else if(from==="library"){model.restore({player:{x:13,y:4,facing:"down"},follower:{x:13,y:3,facing:"down"}});}else{model.restore({player:{x:4,y:5,facing:"right"},follower:{x:3,y:5,facing:"right"}});}
    closeDialog();render();requestAnimationFrame(updateCamera);if(!silent){const label=id==="school"?"学校":id==="library"?"ピジブルの図書館":"はじまりの町";showDialog({speaker:"system",text:`${label}へ移動した。`});}
  }
  ...
  function interact(){
    ...
    if(fieldState.mapId==="town"&&same(f,town.npcs.traveler)){showDialog({speaker:"traveler",text:"学校とピジブルの図書館は、北側の入口から中へ入れるよ。"});return}
    if(fieldState.mapId==="town"&&same(f,town.homeDoor)){G.openComputer?.();return}
  ...
  function tryMove(direction){
    ...
    if(fieldState.mapId==="town"&&!fieldState.enemyDefeated&&same(next,town.enemy)){render();if(!window.SpellStory?.isComplete?.())showDialog({speaker:"lumiere",text:"魔物は後。まず町の依頼を片付けよう。"});else if(G.magicReady())G.startFieldBattle();else showDialog({speaker:"lumiere",text:"Fire と Repair をまだ修得していない。パソコンを開こう。"});return}
  ...
  function saveGame(){const data={version:10,magic:G.serializeMagic(),party:G.serializeParty(),items:G.serializeItems(),story:window.SpellStory?.serialize?.()||null,field:{...model.snapshot(),mapId:fieldState.mapId,enemyDefeated:fieldState.enemyDefeated,steps:fieldState.steps}};localStorage.setItem("spell-operator-v03",JSON.stringify(data));if(G.screens.field.classList.contains("active"))showDialog({speaker:"system",text:"セーブしました。"});return true;}
