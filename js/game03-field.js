(() => {
  ...
  const W=14,H=10,door={x:2,y:3},npc={x:6,y:4},enemy={x:11,y:5},sign={x:8,y:7};
  const blocked=[];
  ...
  dialogStyle.href="css/dialog-portrait.css?v=1";
  document.head.appendChild(dialogStyle);

  const dialog=$("#field-dialog");
  dialog.innerHTML=`
    <div class="dialog-speaker">
      <div id="field-dialog-portrait" class="dialog-portrait" aria-hidden="true"></div>
      <div id="field-dialog-name" class="dialog-name">SYSTEM</div>
    </div>
    <div class="dialog-message">
      <div id="field-dialog-text"></div>
  ...
  function tryMove(direction){
    if(fieldState.dialogOpen||!G.screens.field.classList.contains("active"))return;
    ...
    if(!fieldState.enemyDefeated&&same(next,enemy)){
      model.player.facing=direction;
      render();
      if(G.magicReady())G.startFieldBattle();
  ...
  function startNewGame(){
    model.restore({player:{x:5,y:7,facing:"up"},follower:{x:5,y:8,facing:"up"}});
    fieldState.enemyDefeated=false;fieldState.dialogOpen=false;fieldState.steps=0;
    closeDialog();render();G.showScreen("field");
    setTimeout(()=>showDialog({speaker:"lumiere",text:"まずは魔法工房で Fire と Heal を準備しよう。私は後ろからついていく。"}),80);
  }
  function returnFromWorkshop(){
    render();G.showScreen("field");
    if(G.magicReady())setTimeout(()=>showDialog({speaker:"lumiere",text:"準備完了。東の草むらで実戦テストしよう。"}),80);
  }
  function onBattleWon(){
    fieldState.enemyDefeated=true;render();G.showScreen("field");
  ...
  function loadGame(){
    try{
      ...
      console.error(e);
      showDialog({speaker:"system",text:"ロードに失敗しました。"});
    }
  }
