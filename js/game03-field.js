(() => {
  ...
  const W=20,H=14;
  const homeDoor={x:2,y:8},shop={x:11,y:11},enemy={x:17,y:5},sign={x:9,y:5};
  ...
  function ensureWorld(){const map=$("#field-map");let world=$("#field-world");if(!world){world=document.createElement("div");world.id="field-world";world.className="field-world";map.insertBefore(world,map.firstChild);}["#field-follower","#field-npc","#field-enemy","#field-sign"].forEach(sel=>{const el=$(sel);if(el&&el.parentElement!==world)world.appendChild(el)});return world;}
  ...
  function addSchoolMarker(world){const el=document.createElement("div");el.className="school-marker";el.textContent="↑ 学校入口";el.style.setProperty("--x",2.15);el.style.setProperty("--y",4.05);world.appendChild(el);}
  ...
  function buildMap(){const world=ensureWorld();[...world.querySelectorAll(".field-tile,.field-map-label,.school-marker,.story-npc:not(#field-npc),#field-shop")].forEach(el=>el.remove());const frag=document.createDocumentFragment();for(let y=0;y<H;y++)for(let x=0;x<W;x++){const t=document.createElement("div");t.className=`field-tile ${tileType(x,y)}`;t.dataset.x=x;t.dataset.y=y;frag.appendChild(t);}world.insertBefore(frag,world.firstChild);addMapLabel(world,"学校",1.25,1.42,4.5,"school-label");addMapLabel(world,"ソフィーの家",1.0,6.45,4.0);addMapLabel(world,"図書館",11.4,1.45,4.2);addMapLabel(world,"パーツ屋",15.1,7.45,3.8);addMapLabel(world,"魔導具店",10.0,10.15,3.0);addSchoolMarker(world);const classmate=$("#field-npc");classmate.classList.add("story-npc","npc-classmate");classmate.setAttribute("aria-label","クラスメイト");world.appendChild(makeNpc("field-librarian","npc-librarian","ピジブル"));world.appendChild(makeNpc("field-parts-owner","npc-parts","パーツ屋の店主"));world.appendChild(makeNpc("field-traveler","npc-traveler","旅人"));const shopEl=document.createElement("div");shopEl.id="field-shop";shopEl.className="field-entity field-shop";shopEl.setAttribute("aria-label","魔導具店");shopEl.innerHTML='<span class="shop-icon">◆</span>';world.appendChild(shopEl);}
  function place(sel,pos){const el=$(sel);if(!el)return;el.style.setProperty("--x",pos.x);el.style.setProperty("--y",pos.y);if(pos.facing)el.dataset.facing=pos.facing;}
  ...
  function renderQuestMarks(){const target=window.SpellStory?.questTarget?.(),marks={classmate:"#field-npc",librarian:"#field-librarian",parts:"#field-parts-owner"};Object.entries(marks).forEach(([k,sel])=>{const el=$(sel),tag=el?.querySelector(".entity-tag");if(!el||!tag)return;tag.textContent=target===k?"!":"";el.classList.toggle("quest-target",target===k);});}
  function render(){place("#field-player",model.player);place("#field-follower",model.follower);place("#field-npc",npcs.classmate);place("#field-librarian",npcs.librarian);place("#field-parts-owner",npcs.parts);place("#field-traveler",npcs.traveler);place("#field-enemy",enemy);place("#field-sign",sign);place("#field-shop",shop);$("#field-enemy")?.classList.toggle("defeated",fieldState.enemyDefeated);renderQuestMarks();updateObjective();window.SpellMenu?.renderFieldMenu?.();updateCamera();}
  function updateObjective(){const storyObjective=window.SpellStory?.objective?.();let text;if(storyObjective)text=storyObjective;else if(fieldState.enemyDefeated)text="実戦成功。町を探索しよう";else if(G.magicReady())text="東の草むらにいる魔物へ向かおう";else text="メニューのパソコンで Fire と Repair の魔導書を解析しよう";$("#field-objective").textContent=text;}
  ...
  function lumiereText(){if(!window.SpellStory?.isComplete?.())return"まずは町の人に話を聞こう。頭上に！が出ている相手が次の手掛かりみたい。";return fieldState.enemyDefeated?"今の戦い、悪くなかった。自分で直した魔法が動くと気分いいね。":G.magicReady()?"Fire と Repair は実戦登録できた。あとは東側で実戦テスト。":"パソコンを開いて魔導書を解析しよう。Fire と Repair の両方を実戦登録しておくといい。";}
  ...
  function interact(){if(window.SpellStory?.isOverlayOpen?.())return;if(fieldState.dialogOpen){closeDialog();return}const f=front();if(same(f,model.follower)){showDialog({speaker:"lumiere",text:lumiereText()});return}if(same(f,npcs.classmate)){window.SpellStory?.handleNpc?.("classmate");return}if(same(f,npcs.librarian)){window.SpellStory?.handleNpc?.("librarian");return}if(same(f,npcs.parts)){window.SpellStory?.handleNpc?.("parts");return}if(same(f,npcs.traveler)){showDialog({speaker:"traveler",text:"この町は一本道じゃないよ。学校、図書館、ソフィーの家、店を歩いて回るといい。"});return}if(same(f,homeDoor)){G.openComputer?.();return}if(same(f,shop)){window.SpellItems?.openShop?.();return}if(same(f,sign)){showDialog({speaker:"sign",text:"← 学校・ソフィーの家　　↑ 図書館　　→ パーツ屋・魔導具店"});return}}
  function tryMove(direction){if(fieldState.dialogOpen||window.SpellStory?.isOverlayOpen?.()||window.SpellMenu?.isOpen?.()||!G.screens.field.classList.contains("active"))return;const d=window.SpellFieldModel.DIRS[direction],next={x:model.player.x+d.x,y:model.player.y+d.y};if(!fieldState.enemyDefeated&&same(next,enemy)){model.player.facing=direction;render();if(!window.SpellStory?.isComplete?.())showDialog({speaker:"lumiere",text:"魔物は後。まず町の依頼を片付けよう。"});else if(G.magicReady())G.startFieldBattle();else showDialog({speaker:"lumiere",text:"待って。Fire と Repair がまだ実戦登録されてない。パソコンを開こう。"});return}const result=model.tryMove(direction);if(result.moved){fieldState.steps++;render()}else render();}
  function startNewGame(){G.showScreen("field");model.restore({player:{x:4,y:5,facing:"right"},follower:{x:3,y:5,facing:"right"}});fieldState.enemyDefeated=false;fieldState.dialogOpen=false;fieldState.steps=0;closeDialog();render();window.SpellStory?.startChapter1?.();requestAnimationFrame(updateCamera);}
  ...
  function saveGame(){const data={version:9,magic:G.serializeMagic(),party:G.serializeParty(),items:G.serializeItems(),story:window.SpellStory?.serialize?.()||null,field:{...model.snapshot(),enemyDefeated:fieldState.enemyDefeated,steps:fieldState.steps}};localStorage.setItem("spell-operator-v03",JSON.stringify(data));if(G.screens.field.classList.contains("active"))showDialog({speaker:"system",text:"セーブしました。"});return true;}
  function loadGame(){try{const raw=localStorage.getItem("spell-operator-v03");if(!raw){showDialog({speaker:"system",text:"セーブデータがありません。"});return}const data=JSON.parse(raw);G.restoreMagic(data.magic||{});G.restoreItems(data.items||{});G.restoreParty(data.party||{});window.SpellStory?.restore?.(data.story);model.restore(data.field||{});fieldState.enemyDefeated=Boolean(data.field?.enemyDefeated);fieldState.steps=Number(data.field?.steps||0);G.showScreen("field");render();requestAnimationFrame(updateCamera);showDialog({speaker:"system",text:"ロードしました。"});}catch(e){console.error(e);showDialog({speaker:"system",text:"ロードに失敗しました。"});}}
  ...
  buildMap();render();document.addEventListener("keydown",keydown);window.addEventListener("resize",()=>requestAnimationFrame(updateCamera));document.querySelectorAll("[data-dir]").forEach(b=>b.addEventListener("click",()=>tryMove(b.dataset.dir)));$("#field-action").addEventListener("click",interact);$("#field-save").addEventListener("click",saveGame);$("#field-load").addEventListener("click",loadGame);window.SpellField={startNewGame,returnFromWorkshop,onBattleWon,showDialog,updateObjective,renderQuestMarks,saveGame,loadGame};
})();
