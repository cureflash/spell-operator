(() => {
  ...
  function addMapLabel(map,text,x,y,w){const el=document.createElement("div");el.className="field-map-label";el.textContent=text;el.style.setProperty("--x",x);el.style.setProperty("--y",y);el.style.setProperty("--w",w);map.appendChild(el);}
  ...
  function buildMap(){
    const map=$("#field-map");[...map.querySelectorAll(".field-tile,.field-map-label,.story-npc:not(#field-npc),#field-shop")].forEach(el=>el.remove());
    ...
    addMapLabel(map,"学校",1.3,1.45,4.4);addMapLabel(map,"魔法工房",1.0,6.45,4.0);addMapLabel(map,"図書館",11.4,1.45,4.2);addMapLabel(map,"パーツ屋",15.1,7.45,3.8);addMapLabel(map,"魔導具店",10.0,10.15,3.0);
    const classmate=$("#field-npc");classmate.classList.add("story-npc","npc-classmate");classmate.setAttribute("aria-label","クラスメイト");
    map.appendChild(makeNpc("field-librarian","npc-librarian","ピジブル"));map.appendChild(makeNpc("field-parts-owner","npc-parts","パーツ屋の店主"));map.appendChild(makeNpc("field-traveler","npc-traveler","旅人"));
    const shopEl=document.createElement("div");shopEl.id="field-shop";shopEl.className="field-entity field-shop";shopEl.setAttribute("aria-label","魔導具店");shopEl.innerHTML='<span class="shop-icon">◆</span>';map.appendChild(shopEl);
  }
  ...
  function renderQuestMarks(){const target=window.SpellStory?.questTarget?.();const marks={classmate:"#field-npc",librarian:"#field-librarian",parts:"#field-parts-owner"};Object.entries(marks).forEach(([k,sel])=>{const el=$(sel),tag=el?.querySelector(".entity-tag");if(!el||!tag)return;tag.textContent=target===k?"!":"";el.classList.toggle("quest-target",target===k);});}
  ...
  function updateObjective(){const storyObjective=window.SpellStory?.objective?.();let text;if(storyObjective)text=storyObjective;else if(fieldState.enemyDefeated)text="実戦成功。町を探索しよう";else if(G.magicReady())text="東の草むらにいる魔物へ向かおう";else text="西側の魔法工房で Fire と Heal を登録しよう";$("#field-objective").textContent=text;}
  ...
  function lumiereText(){if(!window.SpellStory?.isComplete?.())return"まずは町の人に話を聞こう。頭上に！が出ている相手が次の手掛かりみたい。";return fieldState.enemyDefeated?"今の戦い、悪くなかった。自分で書いた魔法が動くと、ちょっと気分いいね。":G.magicReady()?"Fire と Heal は登録できた。あとは東側で実戦テスト。":"次は魔法工房。戦闘中にコードを書くわけじゃないから、使う魔法は先に準備しておこう。"}
  ...
  function interact(){
    ...
    if(same(f,door)){if(window.SpellStory?.isComplete?.())G.openWorkshop();else showDialog({speaker:"lumiere",text:"工房は後でもいい。今はクラスメイトの暗号の方を片付けよう。"});return}
  ...
  function tryMove(direction){
    ...
    if(!fieldState.enemyDefeated&&same(next,enemy)){model.player.facing=direction;render();if(!window.SpellStory?.isComplete?.())showDialog({speaker:"lumiere",text:"魔物は後。まず町の依頼を片付けよう。"});else if(G.magicReady())G.startFieldBattle();else showDialog({speaker:"lumiere",text:"待って。まだ実戦用の魔法が揃ってない。工房へ戻ろう。"});return}
    const result=model.tryMove(direction);if(result.moved){fieldState.steps++;render()}else render();
  }

  function startNewGame(){model.restore({player:{x:4,y:5,facing:"right"},follower:{x:3,y:5,facing:"right"}});fieldState.enemyDefeated=false;fieldState.dialogOpen=false;fieldState.steps=0;closeDialog();render();if(window.SpellStory?.startChapter1)window.SpellStory.startChapter1();else G.showScreen("field");render();}
  function returnFromWorkshop(){render();G.showScreen("field")}
  function onBattleWon(){fieldState.enemyDefeated=true;render();G.showScreen("field");setTimeout(()=>showDialog({speaker:"system",text:"グリッチスライムを倒した。"}),80)}

  function saveGame(){const data={version:7,magic:G.serializeMagic(),party:G.serializeParty(),items:G.serializeItems(),story:window.SpellStory?.serialize?.()||null,field:{...model.snapshot(),enemyDefeated:fieldState.enemyDefeated,steps:fieldState.steps}};localStorage.setItem("spell-operator-v03",JSON.stringify(data));if(G.screens.field.classList.contains("active"))showDialog({speaker:"system",text:"セーブしました。"});return true;}
  function loadGame(){try{const raw=localStorage.getItem("spell-operator-v03");if(!raw){showDialog({speaker:"system",text:"セーブデータがありません。"});return}const data=JSON.parse(raw);G.restoreMagic(data.magic||{});G.restoreItems(data.items||{});G.restoreParty(data.party||{});window.SpellStory?.restore?.(data.story);model.restore(data.field||{});fieldState.enemyDefeated=Boolean(data.field?.enemyDefeated);fieldState.steps=Number(data.field?.steps||0);render();G.showScreen("field");showDialog({speaker:"system",text:"ロードしました。"})}catch(e){console.error(e);showDialog({speaker:"system",text:"ロードに失敗しました。"})}}
