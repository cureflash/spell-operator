(() => {
  ...
  const HOUSE1={width:20,height:15};
  const HOUSE2={width:20,height:15};
  const town={
  ...
  const house1={exit:{x:9,y:14},stairs:{x:17,y:2},mother:{x:7,y:6,facing:"down"}};
  const house2={stairs:{x:17,y:13},pc:{x:15,y:3}};
  ...
  function house1Blocked(){
    ...
    addRect(b,2,2,6,3);          // kitchen counter
    addRect(b,4,7,8,8);          // dining table
    addRect(b,11,6,15,7);        // sofa / living furniture
    addRect(b,15,4,18,5);        // cabinet area
    b.add(key(house1.mother.x,house1.mother.y));
    return b;
  }
  function house2Blocked(){
    const b=new Set();addBorder(b,HOUSE2.width,HOUSE2.height);
    addRect(b,2,2,5,4);          // Sophie's bed side
    addRect(b,7,2,10,4);         // Lumiere's bed side
    addRect(b,13,2,17,3);        // desks / PC
    addRect(b,2,8,4,9);          // dresser
    addRect(b,15,7,17,9);        // wardrobe
    return b;
  }
  ...
  function buildMap(){
    ...
    if(fieldState.mapId==="town"){
      ...
      addMapLabel(world,"ソフィーの家 1F",6.2,.55,7.6,"house-label");addMarker(world,"▼ 外へ",8.1,13.15,"house-marker");addMarker(world,"▲ 2F",16.1,2.9,"house-marker");world.appendChild(makeNpc("field-mother","npc-mother","お母さん"));
    }else if(fieldState.mapId==="house2"){
      addMapLabel(world,"ソフィーとルミエルの部屋",5.0,.55,10.0,"house-label");addMarker(world,"▼ 1F",16.0,12.0,"house-marker");addMarker(world,"PC",14.25,4.0,"house-marker house-pc-marker");
    }
  }
  ...
  function placeMapEntities(){
    if(fieldState.mapId==="town"){place("#field-parts-owner",town.npcs.parts);place("#field-traveler",town.npcs.traveler);place("#field-enemy",town.enemy);place("#field-sign",town.sign);place("#field-shop",town.shop);$("#field-enemy")?.classList.toggle("hidden",false);$("#field-sign")?.classList.toggle("hidden",false);}
    else{$("#field-enemy")?.classList.add("hidden");$("#field-sign")?.classList.add("hidden");}
    if(fieldState.mapId==="school")place("#field-npc",school.npcs.classmate);
    if(fieldState.mapId==="library"){place("#field-librarian",library.npcs.librarian);if(G.inventoryCount("unicodeChart")===0)place("#field-unicode-item",library.unicode);}
    if(fieldState.mapId==="house1")place("#field-mother",house1.mother);
  }
  function updateCamera(){const map=$("#field-map"),world=$("#field-world");if(!map||!world)return;const rect=map.getBoundingClientRect();if(rect.width<1||rect.height<1)return;const tile=parseFloat(getComputedStyle(map).getPropertyValue("--tile-size"))||40;world.style.transform=`translate3d(${rect.width/2-(model.player.x+.5)*tile}px,${rect.height/2-(model.player.y+.5)*tile}px,0)`;}
  ...
  function activateMap(id,{snapshot=null,from=null,silent=false}={}){
    fieldState.mapId=id;const spec=mapSpec(id);model.width=spec.width;model.height=spec.height;model.blocked=new Set(spec.blocked);buildMap();
    if(snapshot){
      model.restore(snapshot);
    }else if(id==="school"){
      model.restore({player:{x:7,y:10,facing:"up"},follower:{x:7,y:11,facing:"up"}});
    }else if(id==="library"){
      model.restore({player:{x:7,y:10,facing:"up"},follower:{x:7,y:11,facing:"up"}});
    }else if(id==="house1"&&from==="house2"){
      model.restore({player:{x:17,y:4,facing:"up"},follower:{x:17,y:5,facing:"up"}});
    }else if(id==="house1"){
      model.restore({player:{x:9,y:13,facing:"up"},follower:{x:9,y:14,facing:"up"}});
    }else if(id==="house2"){
      model.restore({player:{x:17,y:12,facing:"up"},follower:{x:17,y:13,facing:"up"}});
    }else if(from==="school"){
      model.restore({player:{x:3,y:4,facing:"down"},follower:{x:3,y:3,facing:"down"}});
    }else if(from==="library"){
      model.restore({player:{x:13,y:4,facing:"down"},follower:{x:13,y:3,facing:"down"}});
    }else if(from==="house1"){
      model.restore({player:{x:2,y:9,facing:"down"},follower:{x:2,y:8,facing:"down"}});
    }else{
      model.restore({player:{x:4,y:5,facing:"right"},follower:{x:3,y:5,facing:"right"}});
    }
    closeDialog();render();requestAnimationFrame(updateCamera);
  }
  function transitionIfNeeded(next){
    if(fieldState.mapId==="town"&&same(next,town.schoolDoor)){activateMap("school",{from:"town"});return true;}
    if(fieldState.mapId==="town"&&same(next,town.libraryDoor)){activateMap("library",{from:"town"});return true;}
    if(fieldState.mapId==="town"&&same(next,town.homeDoor)){activateMap("house1",{from:"town"});return true;}
    if(fieldState.mapId==="school"&&same(next,school.exit)){activateMap("town",{from:"school"});return true;}
    if(fieldState.mapId==="library"&&same(next,library.exit)){activateMap("town",{from:"library"});return true;}
    if(fieldState.mapId==="house1"&&same(next,house1.exit)){activateMap("town",{from:"house1"});return true;}
    if(fieldState.mapId==="house1"&&same(next,house1.stairs)){activateMap("house2",{from:"house1"});return true;}
    if(fieldState.mapId==="house2"&&same(next,house2.stairs)){activateMap("house1",{from:"house2"});return true;}
    return false;
  }
  ...
  function interact(){
    if(window.SpellStory?.isOverlayOpen?.())return;if(fieldState.dialogOpen){closeDialog();return}const f=front();
    if(same(f,model.follower)){showDialog({speaker:"lumiere",text:lumiereText()});return}
    if(fieldState.mapId==="school"&&same(f,school.npcs.classmate)){window.SpellStory?.handleNpc?.("classmate");return}
    if(fieldState.mapId==="library"&&same(f,library.npcs.librarian)){window.SpellStory?.handleNpc?.("librarian");return}
    if(fieldState.mapId==="library"&&same(f,library.unicode)&&G.inventoryCount("unicodeChart")===0){takeUnicodeChart();return}
    if(fieldState.mapId==="house1"&&same(f,house1.mother)){showDialog({speaker:"mother",text:"おかえり、ソフィー。ルミエルちゃんもいらっしゃい。\n2階を使うなら、散らかしっぱなしにしないのよ。"});return}
    if(fieldState.mapId==="house2"&&same(f,house2.pc)){G.openComputer?.();return}
    if(fieldState.mapId==="town"&&same(f,town.npcs.parts)){window.SpellStory?.handleNpc?.("parts");return}
    if(fieldState.mapId==="town"&&same(f,town.npcs.traveler)){showDialog({speaker:"traveler",text:"学校とピジブルの図書館は北側。ソフィーの家は西側だよ。"});return}
    if(fieldState.mapId==="town"&&same(f,town.shop)){window.SpellItems?.openShop?.();return}
    if(fieldState.mapId==="town"&&same(f,town.sign)){showDialog({speaker:"sign",text:"← 学校・ソフィーの家　　↑ ピジブルの図書館　　→ パーツ屋・魔導具店"});return}
    if(fieldState.mapId==="town"&&same(f,town.schoolDoor)){activateMap("school",{from:"town"});return}
    if(fieldState.mapId==="town"&&same(f,town.libraryDoor)){activateMap("library",{from:"town"});return}
    if(fieldState.mapId==="town"&&same(f,town.homeDoor)){activateMap("house1",{from:"town"});return}
    if(fieldState.mapId==="school"&&same(f,school.exit)){activateMap("town",{from:"school"});return}
    if(fieldState.mapId==="library"&&same(f,library.exit)){activateMap("town",{from:"library"});return}
    if(fieldState.mapId==="house1"&&same(f,house1.exit)){activateMap("town",{from:"house1"});return}
    if(fieldState.mapId==="house1"&&same(f,house1.stairs)){activateMap("house2",{from:"house1"});return}
    if(fieldState.mapId==="house2"&&same(f,house2.stairs)){activateMap("house1",{from:"house2"});return}
  }
  function tryMove(direction){
    if(fieldState.dialogOpen||window.SpellStory?.isOverlayOpen?.()||window.SpellMenu?.isOpen?.()||!G.screens.field.classList.contains("active"))return;const d=window.SpellFieldModel.DIRS[direction],next={x:model.player.x+d.x,y:model.player.y+d.y};model.player.facing=direction;
    if(transitionIfNeeded(next))return;
    if(fieldState.mapId==="town"&&!fieldState.enemyDefeated&&same(next,town.enemy)){render();if(!window.SpellStory?.isComplete?.())showDialog({speaker:"lumiere",text:"魔物は後。まず町の依頼を片付けよう。"});else if(G.magicReady())G.startFieldBattle();else showDialog({speaker:"lumiere",text:"Fire と Repair をまだ修得していない。家の2階でパソコンを開こう。"});return}
  ...
  function saveGame(){const data={version:11,magic:G.serializeMagic(),party:G.serializeParty(),items:G.serializeItems(),story:window.SpellStory?.serialize?.()||null,field:{...model.snapshot(),mapId:fieldState.mapId,enemyDefeated:fieldState.enemyDefeated,steps:fieldState.steps}};localStorage.setItem("spell-operator-v03",JSON.stringify(data));if(G.screens.field.classList.contains("active"))showDialog({speaker:"system",text:"セーブしました。"});return true;}
