(() => {
  ...
  function render(){const b=state.battle;if(!b)return;bar("#enemy-hp-bar","#enemy-hp-text",b.enemy.hp,b.enemy.maxHp);bar("#sophie-hp-bar","#sophie-hp-text",b.sophie.hp,b.sophie.maxHp);bar("#lumiere-hp-bar","#lumiere-hp-text",b.lumiere.hp,b.lumiere.maxHp);bar("#lumiere-mp-bar","#lumiere-mp-text",b.lumiere.mp,b.lumiere.maxMp);$("#enemy-name").textContent=`${b.enemy.name} Lv.${b.enemy.level}`;const sn=$("#sophie-battle-name"),ln=$("#lumiere-battle-name");if(sn)sn.textContent=`ソフィー Lv.${b.sophie.level}`;if(ln)ln.textContent=`ルミエル Lv.${b.lumiere.level}`}
  ...
  function makePartyMember(key){
    ...
    const member={key,name:species.name,level:progress.level,stats,hp:Math.max(0,Math.min(progress.hp,stats.hp)),maxHp:stats.hp};
    if(key==="lumiere"){member.maxMp=G.maxMpFor(stats,progress.level);member.mp=Math.max(0,Math.min(progress.mp,member.maxMp))}
    return member;
  }
  ...
  function syncPartyVitals(){
    ...
    state.party.sophie.hp=Math.max(0,Math.min(b.sophie.hp,b.sophie.maxHp));
    state.party.lumiere.hp=Math.max(0,Math.min(b.lumiere.hp,b.lumiere.maxHp));
    state.party.lumiere.mp=Math.max(0,Math.min(b.lumiere.mp,b.lumiere.maxMp));
  }
  ...
  function startFieldBattle(){
    ...
    $("#enemy-sprite").textContent=state.battle.enemy.symbol;$("#fire-cost").textContent=`MP ${fireSpell.mpCost}`;$("#heal-cost").textContent=`MP ${healSpell.mpCost}`;log(`${state.battle.enemy.name}が あらわれた！\nソフィーはどうする？`);menu("sophie");render();G.showScreen("battle");return true;
  }
  ...
  async function doLumiereAction(action){const b=state.battle;if(b.lumiere.hp<=0)return;if(action.type==="fight"){const dmg=physicalDamage(b.lumiere,b.enemy,PHYSICAL_POWER.lumiere);b.enemy.hp-=dmg;anim(".lumiere-battle","cast");anim("#enemy-sprite","hit");log(`ルミエルの こうげき！\n${b.enemy.name}に ${dmg} のダメージ！\n……あまり効いていない。`);render();await delay(500);return}if(action.type==="fire"){const s=G.getEquippedSpell("fire");if(!s)return;const dmg=magicDamage(b.lumiere,b.enemy,s.power||60);b.lumiere.mp-=s.mpCost;b.enemy.hp-=dmg;anim(".lumiere-battle","cast");anim("#enemy-sprite","hit");log(`ルミエルは 自作魔法 Fire を実行した！\n${b.enemy.name}に ${dmg} のダメージ！\nMP ${s.mpCost} 消費（${s.steps} steps）。`);render();await delay(600);return}if(action.type==="heal"){const s=G.getEquippedSpell("heal");if(!s)return;const t=b[action.target],before=t.hp;t.hp=Math.min(t.maxHp,t.hp+s.heal);b.lumiere.mp-=s.mpCost;anim(".lumiere-battle","cast");log(`ルミエルは 自作魔法 Heal を実行した！\n${action.target==="sophie"?"ソフィー":"ルミエル"}のHPが ${t.hp-before} 回復した。\nMP ${s.mpCost} 消費（${s.steps} steps）。`);render();await delay(600);return}log("ルミエルは みをまもっている。");await delay(300)}
  ...
  function growthData(key,beforeLevel,afterLevel){const species=G.partySpecies[key],before=G.calculateStats(species.baseStats,beforeLevel),after=G.calculateStats(species.baseStats,afterLevel),growth=Object.keys(STAT_LABELS).map(stat=>[STAT_LABELS[stat],after[stat]-before[stat]]).filter(([,value])=>value>0);if(key==="lumiere"){const beforeMp=G.maxMpFor(before,beforeLevel),afterMp=G.maxMpFor(after,afterLevel);if(afterMp>beforeMp)growth.splice(1,0,["MP",afterMp-beforeMp])}return growth}
  ...
  async function win(){
    ...
    const oldSMax=b.sophie.maxHp,oldLMax=b.lumiere.maxHp,oldLMp=b.lumiere.maxMp;
    syncPartyVitals();
    const s=G.addExp("sophie",exp),l=G.addExp("lumiere",exp);
    const newS=G.getMemberStats("sophie"),newL=G.getMemberStats("lumiere"),newLMp=G.maxMpFor(newL,state.party.lumiere.level);
