(() => {
  ...
  const state={selectedSpellKey:null,drafts:Object.create(null),lastRun:null,registeredSpells:Object.create(null),spellSlots:Array(SPELL_SLOT_COUNT).fill(null),battle:null,busy:false,pendingActions:{sophie:null,lumiere:null},party:null,money:START_MONEY,inventory:Object.create(null),equipment:{sophie:null,lumiere:null}};

  function equipmentBonus(memberKey){
    const itemKey=state.equipment?.[memberKey],item=itemDefinitions[itemKey];
    return item?.type==="equipment"?item:null;
  }
  function getMemberStats(memberKey){
    const progress=state.party[memberKey],species=partySpecies[memberKey];
    const stats=calculateStats(species.baseStats,progress.level);
    const equip=equipmentBonus(memberKey);
    if(equip?.stat&&Object.prototype.hasOwnProperty.call(stats,equip.stat))stats[equip.stat]+=equip.bonus||0;
    return stats;
  }
  ...
  function restoreItems(data){
    state.money=Math.max(0,Math.floor(data?.money??START_MONEY));
    state.inventory=Object.assign(Object.create(null),data?.inventory||{});
    state.equipment={sophie:null,lumiere:null,...(data?.equipment||{})};
    for(const key of ["sophie","lumiere"]){if(!itemDefinitions[state.equipment[key]]||itemDefinitions[state.equipment[key]].type!=="equipment")state.equipment[key]=null;}
    normalizeVitals("sophie");normalizeVitals("lumiere");
  }
  ...
  function equipItem(memberKey,itemKey){
    ...
    removeItem(itemKey,1);if(old)addItem(old,1);state.equipment[memberKey]=itemKey;normalizeVitals(memberKey);return true;
  }
  ...
  function useItem(memberKey,itemKey){
  ...
  function unequipSlot(index){if(!Number.isInteger(index)||index<0||index>=SPELL_SLOT_COUNT)return false;state.spellSlots[index]=null;window.SpellMenu?.renderLoadout?.();updateWorkshop();return true}
  ...
  function resetAll(){state.selectedSpellKey=null;state.drafts=Object.create(null);state.lastRun=null;state.registeredSpells=Object.create(null);state.spellSlots=Array(SPELL_SLOT_COUNT).fill(null);state.battle=null;state.busy=false;state.pendingActions={sophie:null,lumiere:null};resetItems();resetParty();updateWorkshop()}
  function updateWorkshop(){const n=Object.keys(state.registeredSpells).length;$("#workshop-progress").textContent=`${n} / 2 開発`;$("#workshop-progress").className=`badge ${n===2?"success":"muted"}`;for(const key of ["fire","heal"]){const badge=$(`#${key}-status`),spell=state.registeredSpells[key];badge.textContent=spell?`${isSpellEquipped(key)?"登録中":"開発済み"} / MP ${spell.mpCost}`:"未開発";badge.className=spell?"badge success":"badge muted"}window.SpellMenu?.renderLoadout?.();window.SpellField?.updateObjective?.()}
  ...
  function restoreMagic(data){state.drafts=Object.assign(Object.create(null),data?.drafts||{});state.registeredSpells=Object.assign(Object.create(null),data?.registeredSpells||{});state.spellSlots=Array(SPELL_SLOT_COUNT).fill(null);if(Array.isArray(data?.spellSlots)){for(let i=0;i<SPELL_SLOT_COUNT;i++){const key=data.spellSlots[i];if(key&&state.registeredSpells[key]&&!state.spellSlots.includes(key))state.spellSlots[i]=key}}else{Object.keys(state.registeredSpells).slice(0,SPELL_SLOT_COUNT).forEach((key,i)=>state.spellSlots[i]=key)}updateWorkshop()}

  window.SpellGame03={$,screens,state,interpreter,spellDefinitions,partySpecies,itemDefinitions,START_LEVEL,SPELL_SLOT_COUNT,START_MONEY,calculateStats,getMemberStats,maxMpFor,expToNext,addExp,serializeParty,restoreParty,serializeItems,restoreItems,inventoryCount,addItem,removeItem,buyItem,equipItem,unequipItem,useItem,isSpellEquipped,getEquippedSpell,equipSpell,unequipSlot,showScreen,openWorkshop,updateWorkshop,magicReady,serializeMagic,restoreMagic,normalizeVitals};
